...
options = {
  ...
  map_frame = "map",
  tracking_frame = "base_link",   -- 【实验一核心】因为没有 IMU，所以追踪底盘中心
  published_frame = "base_link",
  odom_frame = "odom",
  
  -- 是否由 Cartographer 自己提供里程计（也就是它来发布 map -> odom 的 TF）
  provide_odom_frame = true,      -- 【实验一核心】因为底层不发 odom，必须由算法硬算出来
  publish_frame_projected_to_2d = true,
  use_pose_extrapolator = true,
  
  -- 传感器开关控制 (消融实验的变量)
  use_odometry = false,           --【实验一核心】彻底关闭外部轮式里程计
...
-- 2D 轨迹构建器具体参数
TRAJECTORY_BUILDER_2D.use_imu_data = false  -- 【实验一核心】彻底关闭 IMU 融合
